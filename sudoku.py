...
class Board:
    ...
    def reset_to_original(self):  # with reset button
        self.board = [
            [self.initial_board[i][j] for j in range(self.cols)]
            for i in range(self.rows)]
    ...
    def initial_board_clone(self):
        initial_board = [[0 for _ in range(BOARD_COLS)] for _ in range(9)]
        for row in range(self.rows):
            for col in range(self.cols):
    ...
    # works with check_board to ensure that each value is valid

    def is_valid(self, row, col, num):
        # makes sure to pass (x,x) to valid in box
        if self.valid_in_box(int(row - (row % self.box_length)), int(col - (col % self.box_length)), num) and self.valid_in_row(row, num) and self.valid_in_col(col, num):
            return True
...
def draw_game_won_screen(screen, exit_surf):
    ...
    # display game won text; exit button is passed in as parameter exit_surf for easier implementation
    ...
    while True:                             # exit button functionality
        for event in pygame.event.get():
...
def draw_game_over_screen(screen, restart_surf):
    ...
    # display game over text; restart button is passed in as parameter restart_surf for easier implementation
    ...
    while True:                             # restart button functionality
        for event in pygame.event.get():
...
if __name__ == '__main__':
    ...
    while True:
        cor = False                     # cor stores the coordinate of user click, initialized as False to avoid errors
        selected_cell_value = 0         # stores the value of selected cell to help with drawing input on the board
        game_over = False               # determines when to return to game start screem

        diff = draw_start_screen(screen)
        screen.fill(BG_COLOR)
        board = Board(WIDTH, HEIGHT, screen, diff)

        button_dict = board.draw(True)  # will store game buttons for use in main screen
        ...
        while True and not game_over:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
                    ...
                    if button_dict["RESET"].collidepoint(event.pos):    # reset board
                ...
                if event.type == pygame.KEYDOWN and cor:  # get number clicked by the user
                    ...
                    if event.key == pygame.K_1 and board.can_place(cor[0], cor[1]):
                        board.sketch(1, cor[0], cor[1])                 # sketches value on selected cell
                    ...
                    if event.key == pygame.K_BACKSPACE and board.can_place(cor[0], cor[1]):
                        ...
                        board.select(cor[0], cor[1])    # leave cell selected

                    # use arrow keys to alternate between cells

                    if event.key == pygame.K_UP:
                        if cor[0] != 0:             # prevents user from going 'beyond' board
                            board.deselect()        # deselects previous cell
                            cor[0] -= 1
                            board.select(cor[0], cor[1])
                            selected_cell_value = board.cells[cor[0]][cor[1]].sketched_value    # selects new cell
                        else:
                            board.select(cor[0], cor[1])
                ...
                if board.is_full():
                    if board.check_board():
                        pygame.time.delay(1000)  # delay update to show board clearly
